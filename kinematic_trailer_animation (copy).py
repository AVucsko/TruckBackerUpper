...
def sm_controller(dt):
    ...
    vel = -1
    L2 = 16.15
    ooga = -np.cos(trailer.yaw)/(vel*L2) + k1x * trailer.x/vel
    if ooga > 1:
        ooga = 1
    elif ooga < -1:
        ooga = -1

    booga = -k1y*trailer.yaw*L2/vel
    if booga > 1:
        booga = 1
    elif booga < -1:
        booga = -1
    theta_x = np.arcsin(ooga)
    theta_y = np.arcsin(booga) + trailer.yaw
...
def main():
    ...
    if maneuver_choice == 'offset':
        ...
        tractor.x = 3.65 # Error measured at kingpin (meters)
        tractor.y = 40 # Error measured at kingpin (meters)
        trailer.x = tractor.x
        trailer.y = tractor.y - 16.15
        ...
        dt = 0.1 #seconds
        steps_max = int(np.floor(sim_time / dt)) #iterations
        tractor.x = 22.15 # Error measured at kingpin (meters)
        tractor.y = 22.15 # Error measured at kingpin (meters)
        trailer.x = tractor.x
        trailer.y = tractor.y - 16.15
    ...
    # Setup Goal Array
    goal_array = np.zeros([steps_max, 1]) # Previously setup to do 1sA???
    ...
    def animate(i):
        ax.cla()
        trail_pat = plt.Rectangle(xy=(simulation_variables[0,2] - 1*math.cos(simulation_variables[0,5]), simulation_variables[0,3] + 1*math.sin(simulation_variables[0,5])), height=16.15, width=2.6, angle=simulation_variables[0,5],color='r')
        trail_pat.rotation_point='center'
        trail_pat.angle = simulation_variables[0,5] * -180/3.14
        tract_pat = plt.Rectangle(xy=(simulation_variables[0,0] - 1*math.cos(simulation_variables[0,4]), simulation_variables[0,1] + 1*math.sin(simulation_variables[0,4])), width=2.6, height=6, angle = simulation_variables[0,4], color='k')
        ...
        trail_pat = plt.Rectangle(xy=(simulation_variables[i,2] - 1*math.cos(simulation_variables[i,5]), simulation_variables[i,3] + 1*math.sin(simulation_variables[i,5])), height=16.15, width=2.6, angle=simulation_variables[i,5],color='r')
        trail_pat.rotation_point='center'
        trail_pat.angle = simulation_variables[i,5] * -180/3.14
        tract_pat = plt.Rectangle(xy=(simulation_variables[i,0] - 1*math.cos(simulation_variables[i,4]), simulation_variables[i,1] + 1*math.sin(simulation_variables[i,4])), width=2.6, height=6, angle = simulation_variables[i,4], color='k')
        tract_pat.rotation_point='center'
