...
def nn_controller():
	...
	# Neurons
    z1 = np.tanh(tractor.yaw * nn.w[0] + trailer1.yaw * nn.w[1] + dolly.yaw * nn.w[2])
    z2 = np.tanh(tractor.x * nn.w[3] + trailer1.x * nn.w[4] + dolly.x * nn.w[5]) 
    z3 = np.tanh(z1*nn.w[6] + z2*nn.w[7])
